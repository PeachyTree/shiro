...
const { GITHUB_ACCESS_TOKEN, SHIRO_GITHUB_REPO_USERNAME, SHIRO_GITHUB_REPO_NAME } = process.env;
...
module.exports = class ChangelogCommand extends Command {
	...
	async run(msg) {
		const { body } = await request
			.get(`https://api.github.com/repos/${SHIRO_GITHUB_REPO_USERNAME}/${SHIRO_GITHUB_REPO_NAME}/commits`)
			.set({ Authorization: `token ${GITHUB_ACCESS_TOKEN}` });
		const commits = body.slice(0, 10);
		const embed = new MessageEmbed()
			.setTitle(`[${SHIRO_GITHUB_REPO_NAME}:master] Latest 10 commits`)
			.setColor(0x7289DA)
			.setURL(`https://github.com/${SHIRO_GITHUB_REPO_USERNAME}/${SHIRO_GITHUB_REPO_NAME}/commits/master`)
