...
class EmojiImage extends Command {
  ...
  async run(message, args, level, settings) { 
    if (!args[0]) return message.react('🚫'), message.reply('Command Usage: `emoji-image <Emoji>`');
    if (args[0].startsWith("<a:")) return message.reply(":warning: This command does not support animated emojis yet.");
