...
const { IMGUR_KEY, SLAP_ALBUM_ID } = process.env; 
...
class Slap extends Command {
    ...
    async run(message, args) {
        ...
        if (!user || !args[1]) {
            return message.reply('Command Usage: `slap <@USER_MENTION>`')
        }

        const image = await this.random();
        if (!image) return message.reply('This album has no images...');
        return message.channel.send(`_**${message.author.username}** slaps **${user.username}**._`, { files: [image] });
    ...
    async random() {
        ...
        const { body } = await request
            .get(`https://api.imgur.com/3/album/${SLAP_ALBUM_ID}`)
